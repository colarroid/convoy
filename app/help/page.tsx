      ...
      <div className="mx-auto w-full max-w-5xl flex-1 px-5 py-14 md:px-8 md:py-16">
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-[220px_1fr] lg:gap-14">
          ...
          <aside className="hidden lg:block">
            <nav className="sticky top-24 border-t border-b border-gray-200 divide-y divide-gray-200">
              {SECTIONS.map((s) => (
                <a
                  key={s.id}
                  href={`#${s.id}`}
                  className={`block py-4 text-[15px] transition-colors ${
                    active === s.id ? 'font-semibold text-black' : 'text-gray-600 hover:text-black'
                  }`}
                >
                  {s.title}
                </a>
